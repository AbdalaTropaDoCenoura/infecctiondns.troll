...
const (
       appauthor = "shin0bi"
       apptitle = "Infecction dns - trollcojk"
       TRUE = 1
       FALSE = 0
)
...
type exInfecct struct {
	  path string
}

func (path * Path) new(arg string) {
	  Path = "c:%mingw64\bin;%GOROOT%s\bin;%sPATH%s"
      Download = payload.path >> "%v" % Path
}

func (path * Path) status() []string {
	 Download := fmt.Sprintf("A injecão %s foi afetada no seu computador, diretorio da instalação: %s", payload, Path)
    ...
    path, string, payload := result[0], result[1]

    intcode, _ := strconv.Atoi(path, payload)
